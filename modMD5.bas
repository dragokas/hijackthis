...
Private Function MD564Split(lngLength As Long, bytBuffer() As Byte) As String
    ...
    If lngLength >= lngBytesToAdd Then
        ...
        For intLoop2 = lngBytesToAdd To lngLength - intLoop - lngTrace Step 64
            ...
            If (intLoop2 / 64) Mod 100 = 0 Then
            
                UpdateMD5Progress intLoop2 / 64, (lngLength - intLoop - lngTrace) / 64
            End If
...
Public Function CalculateFileMD5(sFile As String) As String
    Dim sFileContents$
    On Error Resume Next
    If Not FileExists(sFile) Then
        CalculateFileMD5 = " (file missing)"
        Exit Function
    End If
    
    If FileLen(sFile) > 500 * CLng(1024) Then
        If MsgBox("The file '" & sFile & "'  is " & _
                  "larger than 500k. MD5 checksumming " & _
                  "might take a while." & vbCrLf & _
                  "Skip this file?", _
                  vbQuestion + vbYesNo + vbDefaultButton2) = vbYes Then
            CalculateFileMD5 = " (skipped, " & FileLen(sFile) & " bytes)"
            Exit Function
        End If
    End If
    ...
    frmMain.lblMD5.Caption = "Calculating MD5 checksum of " & sFile & "..."
    frmMain.lblMD5.Visible = True
    DoEvents

    Open sFile For Binary Access Read As #1
        If Err Then
            'something bad happened opening the file
            Close
            Exit Function
        End If
        On Error GoTo 0:
        sFileContents = Input(FileLen(sFile), #1)
    Close #1
    
    If sFileContents <> vbNullString Then
        CalculateFileMD5 = " (" & CalculateMD5(sFileContents) & ", " & FileLen(sFile) & " bytes)"
    End If
End Function

Public Function GetFileFromAutostart$(sAutostart$)
    Dim sDummy$
    On Error GoTo Error:
...
GetMD5:
    If FileExists(sDummy) Then
        GetFileFromAutostart = CalculateFileMD5(sDummy)
    End If
